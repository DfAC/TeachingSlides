...
During the fist week you will be provided with all relevant data files, which you should copy to local folder on your computer. Please make a backup of all your work before leaving the Photogrammetry Lab. This should include all RAW data files and any \emph{Inertial Explorer} generated files. 
\subsection{Data}
Data required for this practical (three types of IMU and GNSS) have been already collected by \textbf{H24VLP} staff and converted to \emph{Inertial Explorer }native format. Those files are:
	\begin{description}
...
Following section explains basics of using IE. For more information please refer to Inertial Explorer Manual provided.
...
\subsection*{NOTES}
To avoid any confusion choose proper antenna model and select ht 0.00. Note that, if you followed instruction correctly, your GNSS and IMU level arm will be calculated to antenna ARP but output level arm will be estimated to antenna phase centre . This means that for export only your level arm has to be corrected using equation $Arm_{exprt}^{Z}=Arm^{z}-AntProfile_{L1}^{z}$. Relevant Arm values can be read from \tabref{Lever-Arm-Values} and antenna profile from the IE (\textbf{View->Project Overview}).

\section{Processing GNSS data\label{sec:Processing-GNSS-data}}

First check your data overlap using \textbf{\emph{Output->Plot Results}}and selecting\emph{ File Data Coverage }plot. 

\subsection{Precise Clocks/Orbits}
The precise orbits/clocks latency is 12-18 days and the GPS rapid ones its less then a day. If observation are available and you haven't downloaded them in last step, use\emph{ }\textbf{\emph{File->Add precise Files}} 
\begin{itemize}
	...
	GLONASS ones);}
	\item Press OK to exit window In \textbf{\emph{View->Project Overview}} you should be able to see all the files you added.
\end{itemize}
...
Successful processing will generate {*}.cmb file (combined GPS file), which is used in Loosely Coupled (LC) solution. To check quality of the position use following plots: 
...
You can also get overview summary using \textbf{\emph{View->Processing summary}}. 

\subsection{Introducing static data in GNSS processing}

IE will automatically detect longer static periods, but for the initialisation of IMU it is advisable to identify static periods at the beginning and end of dataset. To do so:
...
To determine which processing option to use plot \emph{File Data Coverage} and \emph{Velocity profile}. Before proceeding check orientation of axis and level arms (those are values AFTER the rotation) are as provided in \tabref{Lever-Arm-Values}. SPAN GNSS receiver is more advanced then POSRS one. I suggest using SPAN GNSS data for all IMU datasets.
...
\begin{enumerate}
	...
	\item Plot \emph{Acceleration Profile} and \emph{Body Frame Acceleration} to check for suspected static periods using GPS and IMU data respectively. Make sure that dataset start and finish on static period (changing\emph{ IMU Time Range} in \textbf{\emph{Advanced...}} if necessary). You can also check \emph{Velocity Profile} and \emph{Body Frame Velocity.}
	...
	\item Transfer alignment. Use reverse value for forward solution and vice versa. Follow description in \subref{SPAN-Workflow}. Results should be within 10 arc-sec for heading and 2 arc-sec for rest. and no separation between pitch and roll. Heading separation should be largest in the middle and no jumps at the beginning/end. 
\end{enumerate}
...
SPAN data should be processed after POSRS as POSRS results can be used to increase its accuracy. Please created a separate project file for SPAN (it can be in the same folder) following \secref{Processing-GNSS-data}. There is no need to process GNSS data again, in \textbf{\emph{Process->Process LC }}select
...
After defining starting and ending orientation for IMU (using \emph{Transfer Alignment}) it is possible to process IMU only data. To do so:
\begin{itemize}
...
As a part of your assignment you will simulate a GNSS outage. To do so remove epochs from GNSS generated trajectory, generated in \subref{Processing-POSRS}. Each epoch is saved in following record
...
\subsection*{NOTES}
A very useful tool during this exercise is \textbf{\emph{View->Processing History}}, which you can use to restore previous settings. Note that this will not restore any changes to external files so make sure you always work on the COPY!
...
To output post-processed ASCII go to \textbf{\emph{Output->Export Wizard}} 
\begin{itemize}
...
\subsection*{NOTES}
Lever arms should be calculated to ARP (this won't include any L1/L2 height offsets). If you can use \textbf{\emph{Export Wizard}} make sure that combined solution exist ({*}.cmb). Normally it is generated after each processing run but it can be also created using \textbf{\emph{Process->Combine Solution }}. Use extreme caution if using this option. To convert results to OSGB use Grid InQuest\footnote{\url{http://www.ordnancesurvey.co.uk/docs/gps/grid-inquest-executable.zip}}.
