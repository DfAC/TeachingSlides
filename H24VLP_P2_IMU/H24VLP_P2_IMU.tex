...
%you need to compile this template up to 3 times as some cross-references need to run on ex ones.
...
\begin{itemize}
	\item navigation grade Applanix POSRS unit with digital laser gyros;
	\item tactical grade SPAN LCI with fibre optic gyros;
...
To identify static periods use \emph{Acceleration Profile} or \emph{Velocity Profile} plots. Then create new file Static.txt with each row identifying static period as NAME~START\_TIME~END\_TIME~NOTE as follows\footnote{More information at page 32 of IE manual.}:
		...
		\begin{lstlisting} [language=Matlab,backgroundcolor=\color{uniBlueBgd},frame=single,basicstyle=\footnotesize,numbers=left,numbersep=5pt, numberstyle=\tiny\color{uniBlue}]
			NGI_Start 201200.0 203505.0 Start_NGB_Yard
			NGI_End 221200.0 223505.0 End_NGB_Yard
		\end{lstlisting}
...
\vspace{2cm}
\flushleft{Once you identified static periods (they should be between 5-10 min long) compare results using any other plots that we discussed. You can also use your experience from the first project to fine tune processing parameters such as: excluding poor satellites or change cut off angles\footnote{Be careful with changing KF or Ambiguity Resolution (AR) related parameters. Make sure you mark those changes properly in description.}. Once you happy with results save and exit the project. As an extra precaution make a copy of \textit{GNSS.cmb} and re-naming it, for example as \textit{GoodData.cmb}.}



\chapter{Processing LC GNSS and IMU\label{sec:Processing-Loosely-Coupled}}

The goal of this practical is to understand the advantages and limitations of integrated GNSS and IMU systems. To demonstrate the advantage and to compare performance of IMU component, we will use the same GNSS trajectory for each project - the one created in last section\footnote{By using exactly the same GNSS trajectory your comparison will only focus on the IMU component. Should you need to re-process GNSS data you will have to re-process all LC projects as well.}
...
Aim of the project is to understand the performance of the IMU component. To create an artificial GNSS outage you will remove epochs from GNSS generated trajectory - GNSS.cmb file, \refLoc{sec:GPS-process}. In {*}.cmb file each epoch is represented as:
\bigskip
\begin{lstlisting} [language=Matlab,backgroundcolor=\color{uniBlueBgd},frame=single,basicstyle=\footnotesize,numbers=left,numbersep=5pt, numberstyle=\tiny\color{uniBlue}]
...
%\vspace{2cm}
\NOTE{
You have three \textbf{Processing Directions}: Both, Forward and Reverse. For this analysis it might be more useful to process one way only - most likely using \textit{Forward} option. Think what implication will have using \textit{Both} or \textit{Reverse} option.
}

\section{Remove GNSS trajectory}

If previous approach fail to process, remove all GNSS data but two first and two last epochs\footnote{Make sure that those epochs match your processing range staring/ending epochs.}, \refLoc{sec:SIMUlating-GNSS-outage}. Now process Loosely Coupled Solution, \refLoc{sec:Processing-Loosely-Coupled}. Notice that for forward only solution you need only two first epochs.
...
To output post-processed coordinates as an ASCII file:
\begin{itemize}
	...
	\NOTE{
		...
		Relevant Arm values can be read from table \ref{tab:Lever-Arm-Values}. Antenna profile can be verified using (\textbf{View->Project Overview}).
	}
...
\section{Indicative accuracy of the IMUs}

Note that results below are based on the very limited number of trials and should be treated as indicative only. Those results have been produced using a vehicle in typical road characteristics by artificially removing GNSS observations, as discussed in section \refLoc{Processing-IMU-on-its-own}. The critical factors affecting the accuracy is the movement dynamics (more varied the better). Both the prolong movement with same speed and direction as well as slow, crawling movement will severely degrade IMU performance. Also, the heigh hardly changes.
